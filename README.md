...
通过 Ubuntu 中转，在浏览器与防火墙之间传输文件：
- **上传文件**：选择防火墙设备 → 选择本地文件（最大 100MB）→ 自动 SCP 传输到防火墙 `/data/tmp/`
- **下载文件**：选择防火墙设备 → 输入防火墙上的文件路径 → SCP 下载到本地
- **传输方式**：浏览器 → Django (Ubuntu 临时目录) → SCP → 防火墙，反之亦然
- **密码处理**：自动检测 SCP 密码提示并输入 Ubuntu SSH 密码，无需额外配置
