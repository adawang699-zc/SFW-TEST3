...
def api_interface_down():
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(f"[API] 网口 {interface} DOWN 成功")
            return jsonify({
                'success': True,
                'message': f'网口 {interface} 已 DOWN'
            })
        else:
            logger.warning(f"[API] 网口 {interface} DOWN 失败: {result.stderr}")
            return jsonify({
                'success': False,
                'error': result.stderr
            }), 500
        ...
        logger.exception(f"[API] DOWN 网口异常: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def api_interface_up():
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(f"[API] 网口 {interface} UP 成功")
            return jsonify({
                'success': True,
                'message': f'网口 {interface} 已 UP'
            })
        else:
            logger.warning(f"[API] 网口 {interface} UP 失败: {result.stderr}")
            return jsonify({
                'success': False,
                'error': result.stderr
            }), 500
        ...
        logger.exception(f"[API] UP 网口异常: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
def api_interface_config():
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(f"[API] 网口 {interface} 配置成功: autoneg={autoneg}, speed={speed}, duplex={duplex}")
            return jsonify({
                'success': True,
                'message': f'网口 {interface} 已配置'
            })
        else:
            logger.warning(f"[API] 网口 {interface} 配置失败: {result.stderr}")
            return jsonify({
                'success': False,
                'error': result.stderr
            }), 500
        ...
        logger.exception(f"[API] 配置网口异常: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
