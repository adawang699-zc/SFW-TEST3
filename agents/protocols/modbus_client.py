        ...
        with self.lock:
            ...
            try:
                if function_code == 5:  # 写单个线圈
                    ...
                    response = client.write_coils(address=address, values=values_bool, slave=unit_id)

                elif function_code == 16:  # 写多个寄存器
                    values_int = [int(v) for v in values]
                    response = client.write_registers(address=address, values=values_int, slave=unit_id)
